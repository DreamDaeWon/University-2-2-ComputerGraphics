...
enum ArtType { DWART_DOT, DWART_LINE, DWART_TRIANGLE, DWART_RECTANGLE,DWART_SPIRAL };
...
struct DWArt
{
	...
	bool FirstDraw{}; // 처음에 한 번 그렸는지?
	bool TwoDraw{}; // 두번 째 그렸는지?
...
struct DWSpiral : public DWArt
{
	vector<VertexValue> Vertex;
	vector<VertexValue> VertexColor;
...
GLvoid Create_Circle_Sprial(vector<DWArt*>* pVec, GLfloat x, GLfloat y); // 원하는 벡터에 점 스파이럴 추가

GLvoid Move_Circle_Sprial(vector<DWArt*>* pVec);
...
GLvoid UpdateBuffer(DWArt* pArt)
{
	...
	case DWART_SPIRAL:
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(dynamic_cast<DWSpiral*>(pArt)->Vertex), dynamic_cast<DWSpiral*>(pArt)->Vertex.data(), GL_STREAM_DRAW);


		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pArt->EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(dynamic_cast<DWSpiral*>(pArt)->indexVerTex), dynamic_cast<DWSpiral*>(pArt)->indexVerTex.data(), GL_STREAM_DRAW);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(dynamic_cast<DWSpiral*>(pArt)->VertexColor), dynamic_cast<DWSpiral*>(pArt)->VertexColor.data(), GL_STREAM_DRAW);
...
inline GLvoid InitBuffer(DWArt* pArt)
{
	...
	case DWART_SPIRAL:
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(dynamic_cast<DWSpiral*>(pArt)->Vertex.size() * sizeof(float) * 3), dynamic_cast<DWSpiral*>(pArt)->Vertex.data(), GL_STREAM_DRAW);

		glGenBuffers(1, &pArt->EBO);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pArt->EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, dynamic_cast<DWSpiral*>(pArt)->indexVerTex.size() * sizeof(unsigned int), dynamic_cast<DWSpiral*>(pArt)->indexVerTex.data(), GL_STREAM_DRAW);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
		...
		glBufferData(GL_ARRAY_BUFFER, dynamic_cast<DWSpiral*>(pArt)->VertexColor.size() * sizeof(float)*3, dynamic_cast<DWSpiral*>(pArt)->VertexColor.data(), GL_STREAM_DRAW);
...
GLvoid Create_Circle_Sprial(vector<DWArt*>* pVec, GLfloat x, GLfloat y)
{
	// 객체 생성

	DWArt* New = new DWSpiral{};

	New->CenterX = x;

	New->CenterY = y;

	New->Circle_Rotate = 0.f;

	New->Rotate_Radius = 1.f;

	New->Move_Timer = 0; // 시간 값

	New->eType = DWART_SPIRAL;

	VertexValue NewVertex{};
	VertexValue NewVertexColor{ 1.f,1.f,1.f };

	GLint VertexNum = 0;


	NewVertex.data[0] = sinf(New->Circle_Rotate * (M_PI / 180.0f)) * New->Rotate_Radius;
	NewVertex.data[1] = cosf(New->Circle_Rotate * (M_PI / 180.0f)) * New->Rotate_Radius;

	NewVertex.data[0] += New->CenterX;
	NewVertex.data[1] += New->CenterY + New->NowCenterY;
 
 
			 //디버깅을 위한 출력
	std::cout << "NewVertex Before: " << NewVertex.data[0] << ", " << NewVertex.data[1] << std::endl;
	
	NewVertex.data[0] = CoordinateChangeX(NewVertex.data[0]);
	NewVertex.data[1] = CoordinateChangeY(NewVertex.data[1]);
	
	// 디버깅을 위한 출력
	std::cout << "NewVertex After: " << NewVertex.data[0] << ", " << NewVertex.data[1] << std::endl;
	

	dynamic_cast<DWSpiral*>(New)->Vertex.push_back(NewVertex);
	dynamic_cast<DWSpiral*>(New)->VertexColor.push_back(NewVertexColor);
	dynamic_cast<DWSpiral*>(New)->indexVerTex.push_back(VertexNum);

	pVec->push_back(New);

	InitBuffer(pVec->back());

}


GLvoid Move_Circle_Sprial(vector<DWArt*>* pVec) // 모든 원 스파이럴 돌리기
{
	for (auto& iter : *pVec)
	{
		if (iter->eType == DWART_SPIRAL && !iter->FirstDraw)
		{
			VertexValue NewVertex{};
			VertexValue NewVertexColor{1.f,1.f,1.f};
			GLuint VertexNum = dynamic_cast<DWSpiral*>(iter)->indexVerTex.size();

			NewVertex.data[0] = sinf(iter->Circle_Rotate * (M_PI / 180.0f)) * iter->Rotate_Radius;
			NewVertex.data[1] = cosf(iter->Circle_Rotate * (M_PI / 180.0f)) * iter->Rotate_Radius;

			NewVertex.data[0] += iter->CenterX;
			NewVertex.data[1] += iter->NowCenterY;

			if (iter->Rotate_Radius == 0.f)
			{
				NewVertex.data[1] += iter->CenterY;
			}

			/*if (iter->Circle_Rotate == 0.f || iter->Circle_Rotate == 360.f)
			{
				NewVertex.data[1] += iter->CenterY;
			}*/
			 //디버깅을 위한 출력
			...
			dynamic_cast<DWSpiral*>(iter)->Vertex.push_back(NewVertex);
			dynamic_cast<DWSpiral*>(iter)->VertexColor.push_back(NewVertexColor);
			dynamic_cast<DWSpiral*>(iter)->indexVerTex.push_back(VertexNum);
			...
			{
				iter->Rotate_Radius += 10.f;

				if ((int)iter->Circle_Rotate == 180)
				{
					iter->NowCenterY = iter->CenterY + 10.f;
				}
				else if ((int)iter->Circle_Rotate == 360.f)
				{
					iter->NowCenterY = iter->CenterY;
				}
			}
			...
			if (iter->Rotate_Radius >= 60.f)
			{
				iter->FirstDraw = true;
				iter->Circle_Rotate = 180.f;
			}
			//// GPU 버퍼 업데이트 - 부분 업데이트
			//glBindBuffer(GL_ARRAY_BUFFER, iter->VBO[0]);
			//glBufferSubData(GL_ARRAY_BUFFER, (dynamic_cast<DWSpiral*>(iter)->Vertex.size() - 1) * sizeof(VertexValue), sizeof(VertexValue), &NewVertex);

			//glBindBuffer(GL_ARRAY_BUFFER, iter->VBO[1]);
			//glBufferSubData(GL_ARRAY_BUFFER, (dynamic_cast<DWSpiral*>(iter)->VertexColor.size() - 1) * sizeof(VertexValue), sizeof(VertexValue), &NewVertexColor);

			//// EBO 업데이트
			//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, iter->EBO);
			//glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, (dynamic_cast<DWSpiral*>(iter)->indexVerTex.size() - 1) * sizeof(unsigned int), sizeof(VertexNum), &VertexNum);
		}
		else if (iter->eType == DWART_SPIRAL && !iter->TwoDraw)
		{
			VertexValue NewVertex{};
			VertexValue NewVertexColor{ 1.f,1.f,1.f };
			GLuint VertexNum = dynamic_cast<DWSpiral*>(iter)->indexVerTex.size();
			...
			NewVertex.data[0] += iter->CenterX;
			NewVertex.data[1] += iter->NowCenterY + 120.f;
			...
			if ((int)iter->Circle_Rotate % 180 == 0)
			{
				...
				else if ((int)iter->Circle_Rotate == 0.f)
				{
					iter->NowCenterY = iter->CenterY + 10.f;
				}
			}

			if (iter->Circle_Rotate <= 0.f)
			{
				iter->Circle_Rotate = 360.f;
			}
		...
		// GPU 버퍼 업데이트
		glBindBuffer(GL_ARRAY_BUFFER, iter->VBO[0]);
		glBufferData(GL_ARRAY_BUFFER, dynamic_cast<DWSpiral*>(iter)->Vertex.size() * sizeof(float) * 3, dynamic_cast<DWSpiral*>(iter)->Vertex.data(), GL_STREAM_DRAW);

		glBindBuffer(GL_ARRAY_BUFFER, iter->VBO[1]);
		glBufferData(GL_ARRAY_BUFFER, dynamic_cast<DWSpiral*>(iter)->VertexColor.size() * sizeof(float) * 3, dynamic_cast<DWSpiral*>(iter)->VertexColor.data(), GL_STREAM_DRAW);

		// EBO 업데이트
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, iter->EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, dynamic_cast<DWSpiral*>(iter)->indexVerTex.size() * sizeof(unsigned int), dynamic_cast<DWSpiral*>(iter)->indexVerTex.data(), GL_STREAM_DRAW);
	}
