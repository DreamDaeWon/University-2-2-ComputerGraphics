...
enum ArtType {DWART_START, DWART_DOT, DWART_LINE, DWART_TRIANGLE, DWART_RECTANGLE, DWART_PENTAGON, DWART_END};
enum DrawType { DW_DOT, DW_LINE, DW_TRIANGLE, DW_RECT, DW_PENTAGONTYPE };
...
struct DWPentagon : public DWArt
{
	GLfloat Vertex[5][3]{};
	GLfloat VertexColor[5][3]{};
	GLuint indexVerTex[9]{};

	GLfloat BeforeVertex[5][3]{}; // 보간을 위한 이전 정점 기억
...
GLvoid Create_Pentagon(vector<DWArt*>* pVec, GLfloat x, GLfloat y, GLfloat rx, GLfloat ry); // 원하는 위치에 오각형 추가
...
GLvoid UpdateBuffer(DWArt* pArt)
{
	...
	case DWART_PENTAGON:
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(dynamic_cast<DWPentagon*>(pArt)->Vertex), dynamic_cast<DWPentagon*>(pArt)->Vertex, GL_STREAM_DRAW);


		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pArt->EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(dynamic_cast<DWPentagon*>(pArt)->indexVerTex), dynamic_cast<DWPentagon*>(pArt)->indexVerTex, GL_STREAM_DRAW);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(dynamic_cast<DWPentagon*>(pArt)->VertexColor), dynamic_cast<DWPentagon*>(pArt)->VertexColor, GL_STREAM_DRAW);
...
GLvoid ReBindArt(DWArt* pArt, ArtType eDraw)
{
	...
	case DWART_PENTAGON:
		...
		// 정점 0
		dynamic_cast<DWPentagon*>(pArt)->Vertex[0][0] = CoordinateChangeX(dynamic_cast<DWPentagon*>(pArt)->Vertex[0][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[0][1] = CoordinateChangeY(dynamic_cast<DWPentagon*>(pArt)->Vertex[0][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[0][2] = 0.f;

		// 정점 1
		dynamic_cast<DWPentagon*>(pArt)->Vertex[1][0] = CoordinateChangeX(dynamic_cast<DWPentagon*>(pArt)->Vertex[1][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[1][1] = CoordinateChangeY(dynamic_cast<DWPentagon*>(pArt)->Vertex[1][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[1][2] = 0.f;

		// 정점 2
		dynamic_cast<DWPentagon*>(pArt)->Vertex[2][0] = CoordinateChangeX(dynamic_cast<DWPentagon*>(pArt)->Vertex[2][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[2][1] = CoordinateChangeY(dynamic_cast<DWPentagon*>(pArt)->Vertex[2][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[2][2] = 0.f;

		// 정점 3
		dynamic_cast<DWPentagon*>(pArt)->Vertex[3][0] = CoordinateChangeX(dynamic_cast<DWPentagon*>(pArt)->Vertex[3][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[3][1] = CoordinateChangeY(dynamic_cast<DWPentagon*>(pArt)->Vertex[3][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[3][2] = 0.f;

		// 정점 4
		dynamic_cast<DWPentagon*>(pArt)->Vertex[4][0] = CoordinateChangeX(dynamic_cast<DWPentagon*>(pArt)->Vertex[4][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[4][1] = CoordinateChangeY(dynamic_cast<DWPentagon*>(pArt)->Vertex[4][0]);
		dynamic_cast<DWPentagon*>(pArt)->Vertex[4][2] = 0.f;
...
DWArt* Check_Mouse_Rect(vector<DWArt*>& pvector, GLfloat x, GLfloat y)
{
	for (auto iter : pvector)
	{
		if (iter->eWantType == DWART_DOT)
		{
			if ((iter->CenterX - iter->rx * 0.1f <= x && iter->CenterX + iter->rx * 0.1f >= x) &&
				(iter->CenterY - iter->ry * 0.1f <= y && iter->CenterY + iter->ry * 0.1f >= y))
			{
...
DWArt* Check_Rect_Rect(vector<DWArt*>& pvector, DWArt* _CheckRect)
{
	...
	for (auto& iter : pvector)
	{
		if (iter->eWantType == DWART_DOT && _CheckRect->eWantType == DWART_DOT)
		{
			if (iter == _CheckRect)
			{
				continue;
			}
			if (_CheckRect->CenterX - _CheckRect->rx * 0.1f > iter->CenterX + iter->rx * 0.1f || _CheckRect->CenterX + _CheckRect->rx * 0.1f < iter->CenterX - iter->rx * 0.1f)
			{
				continue; // 사각형의 위치가 사각형과 닿지 않았다면?
			}
			if (_CheckRect->CenterY - _CheckRect->ry * 0.1f > iter->CenterY + iter->ry * 0.1f || _CheckRect->CenterY + _CheckRect->ry * 0.1f < iter->CenterY - iter->ry * 0.1f)
			{
				continue; // 사각형의 위치가 사각형과 닿지 않았다면?
			}
			return iter; // 사각형의 위치가 사각형과 닿았다면?
		}
		else if (iter->eWantType == DWART_DOT)
		{
			if (iter == _CheckRect)
			{
				continue;
			}
			if (_CheckRect->CenterX - _CheckRect->rx > iter->CenterX + iter->rx * 0.1f || _CheckRect->CenterX + _CheckRect->rx < iter->CenterX - iter->rx * 0.1f)
			{
				continue; // 사각형의 위치가 사각형과 닿지 않았다면?
			}
			if (_CheckRect->CenterY - _CheckRect->ry > iter->CenterY + iter->ry * 0.1f || _CheckRect->CenterY + _CheckRect->ry < iter->CenterY - iter->ry * 0.1f)
			{
				continue; // 사각형의 위치가 사각형과 닿지 않았다면?
			}
			return iter; // 사각형의 위치가 사각형과 닿았다면?
		}
		else if (_CheckRect->eWantType == DWART_DOT)
		{
			if (iter == _CheckRect)
			{
				continue;
			}
			if (_CheckRect->CenterX - _CheckRect->rx * 0.1f > iter->CenterX + iter->rx || _CheckRect->CenterX + _CheckRect->rx * 0.1f < iter->CenterX - iter->rx)
			{
				continue; // 사각형의 위치가 사각형과 닿지 않았다면?
			}
			if (_CheckRect->CenterY - _CheckRect->ry * 0.1f > iter->CenterY + iter->ry || _CheckRect->CenterY + _CheckRect->ry * 0.1f < iter->CenterY - iter->ry)
			{
				continue; // 사각형의 위치가 사각형과 닿지 않았다면?
			}
			return iter; // 사각형의 위치가 사각형과 닿았다면?
...
GLvoid Create_Pentagon(vector<DWArt*>* pVec, GLfloat x, GLfloat y, GLfloat rx, GLfloat ry)
{
	DWPentagon* Art = new DWPentagon{};
	...
	// 정점  // 4
	Art->Vertex[4][0] = CoordinateChangeX(x + rx);
	Art->Vertex[4][1] = CoordinateChangeY(y - ry);
	Art->Vertex[4][2] = 0.f;
